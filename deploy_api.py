...
@app.post("/idphoto-all")
async def idphoto_all_in_one(
    ...
    try:
        ...
        bg_img_np = add_background(
            raw_img.standard,
            bgr=color,
            mode=render_choice[render],
        ).astype(np.uint8)

        bg_height, bg_width = bg_img_np.shape[:2]

        typography_arr, typography_rotate = generate_layout_photo(
            input_height=bg_height, input_width=bg_width
        )

        layout_img_np = generate_layout_image(
            bg_img_np, typography_arr, typography_rotate, height=bg_height, width=bg_width
        ).astype(np.uint8)
    except FaceError:
    ...
    # 如果检测到人脸数量等于1, 则返回标准证和高清照结果（png 4通道图像）
        ...
        result_message = {
            "status": True,
            # "image_base64_standard": numpy_2_base64(raw_img.standard),
            "image_base64_bg": numpy_2_base64(bg_img_np),
            "image_base64_layout": numpy_2_base64(layout_img_np)
        }

        # 如果hd为True, 则增加高清照结果（png 4通道图像）
        if hd:
            result_message["image_base64_hd"] = numpy_2_base64(raw_img.hd)
